...
def test_parse_no_longer_drops_negated_criteria():
    """Documents an accepted regression, not desired behaviour.

    The polarity field was removed from the schema, so nothing marks a
    negated criterion as negated and nothing filters one out. These entries
    now reach the UI, where "Absence of nausea" under a "Do you have these?"
    checkbox reads as a double negative -- the bug polarity was added to fix.
    The prompt asks for findings the patient HAS, but an instruction is a
    weaker lever than a filter. If negations come back, this test is the
    place that says why.
    """
    raw = """[
    ...
    assert [c["text"] for c in parse_profile(raw)] == [
        "Bilateral pressing head pain",
        "Absence of nausea or vomiting",
    ]


def test_parse_ignores_an_unexpected_polarity_field():
    """A model may still emit polarity from habit; it must not break parsing."""
    raw = ('[{"description": "Fever", "importance": "strong", '

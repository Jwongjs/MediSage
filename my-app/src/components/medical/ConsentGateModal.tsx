...
import { ShieldCheck, Stethoscope, Lock, UserX } from 'lucide-react';
...
const POINTS = [
  ...
  {
    icon: UserX,
    title: 'No account, nothing saved',
    body: 'There is no login and no stored identity, so we have no way to know whether you agreed before. Your report is generated for this session only and downloads to your own device.',
  },
] as const;

// There is no account and nothing persisted client-side (no localStorage):
// with no identity to attach a prior "agreed" flag to, this gate must be
// shown at the start of every diagnosis workflow session, not just once.
export const ConsentGateModal: React.FC<ConsentGateModalProps> = ({ onAccept, onCancel }) => {
    ...
    <Dialog open onOpenChange={open => { if (!open) onCancel(); }}>
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          <Button onClick={onAccept} disabled={!agreed}>Agree & start assessment</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
